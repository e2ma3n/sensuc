...
function send-mail {
	( echo $t ; echo $msg ) | mailx -v -r "$smtp_user" -s "Sensuc - `echo -n '20' ; date '+%y/%m/%d'`" -S smtp=$smtp_srv -S smtp-use-starttls -S smtp-auth=login -S smtp-auth-user=$smtp_user -S smtp-auth-password=$smtp_pass -S ssl-verify=ignore -S nss-config-dir=/etc/pki/nssdb/ $mail_to &> /dev/null
	if [ "$?" != "0" ] ; then
		echo -n "[-] " >> /opt/sensuc_v1.5/log/errors.log ; echo -n 'DATE: 20' ; date '+%y/%m/%d TIME: %H:%M:%S' >> /opt/sensuc_v1.5/log/errors.log
