...
for (( ;; )) ; do
	...
	su_user=`cat $system_log | grep "\sSuccessful su for\s" | tr -s " " | cut -d " " -f 1,2,3,6,7,8,9,10,11`
	...
	if [ ! -z "$su_user" ] ; then
			...
			for (( i=1 ; i<=$n ; i++ )) ; do
				...
				grep "$log" /opt/sensuc_v1/log/su &> /dev/null
				if [ "$?" != "0" ] ; then
					echo "$log" >> /opt/sensuc_v1/log/su
					echo "$log" | mailx -v -r "$smtp_user" -s "sensuc - `date '+%m/%d/%y'`" -S smtp=$smtp_srv -S smtp-use-starttls -S smtp-auth=login -S smtp-auth-user=$smtp_user -S smtp-auth-password=$smtp_pass -S ssl-verify=ignore -S nss-config-dir=/etc/pki/nssdb/ $mail_to &> /dev/null
					if [ "$?" != "0" ] ; then
